...
pub fn generate_codebase_map(project_path: &str) -> String {
    ...
    let files: Vec<String> = {
        ...
        match output {
            ...
            _ => {
                let skip_dirs = [".git", "node_modules", "target", "dist", "build", ".next", "__pycache__", ".venv", "venv"];
                let mut found = Vec::new();
                let mut stack = vec![base.to_path_buf()];
                while let Some(dir) = stack.pop() {
                    if found.len() >= MAX_FILES { break; }
    ...
    for rel_path in &sorted_files {
        ...
        let dir = p.parent()
            ...
            .map(|s| format!("{}/", s))
            .unwrap_or_default();
        let filename = p.file_name().and_then(|n| n.to_str()).unwrap_or(rel_path).to_string();
    ...
    'outer: for (dir, files) in &grouped {
        ...
        for (name, lines) in files {
            ...
            if output.len() + entry.len() > MAX_CHARS { break 'outer; }
...
pub fn auto_select_model(description: &str, tag: Option<&str>, provider: &str) -> String {
    ...
    let simple = ["fix typo", "rename", "update text", "change name", "remove unused",
                  "add comment", "update comment", "fix import", "fix lint", "formatting",
                  "add docstring", "update readme", "config change"];
    for kw in &simple {
        if desc_lower.contains(kw) { complexity -= 1.0; break; }
    }

    // Complex task indicators (increase complexity)
    let complex = ["implement", "architect", "design", "migrate", "rewrite",
                   "integrate", "overhaul", "multi-file", "across files",
                   "full feature", "from scratch", "complex", "security"];
    for kw in &complex {
        if desc_lower.contains(kw) { complexity += 1.5; break; }
    }
    ...
    let file_mentions: usize = file_extensions.iter()
        .map(|ext| description.matches(ext).count())
        .sum();
    if file_mentions >= 3 { complexity += 1.0; }
    ...
    match provider {
        "codex" => {
            if use_expensive { "gpt-5.4".to_string() } else { "gpt-5.4-mini".to_string() }
        }
        _ => {
            if use_expensive { "claude-opus-4-20250414".to_string() } else { "claude-sonnet-4-20250514".to_string() }
        }
    }
}
